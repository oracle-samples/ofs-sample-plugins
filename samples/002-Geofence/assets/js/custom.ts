        ...
        navigator.geolocation.getCurrentPosition(
          (position) => {
            let distance = haversine(
              ...
              data.activity.acoord_y,
              data.activity.acoord_x
            );
            content.innerHTML =
              content.innerHTML +
              `Geolocation activated: <LI>Tech Position: ${position.coords.latitude.toFixed(
                5
              )}, ${position.coords.longitude.toFixed(
                5
              )} <LI>Distance: ${distance.toFixed(3)} meters`;
